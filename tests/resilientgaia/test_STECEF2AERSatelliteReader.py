...
""")
        ...
        self.satNames_1 = ["G05", "G07", "G09", "G11", "G13", "G15", "G18", "G20", "G30", "R07",
...
""")
    ...
    def testRename_satellites(self):
        actual = STECEF2AERSatelliteReader(self.sio_1, lat="lat", lon="lon",
                                           alt="alt", sep=",", decimal=".",
        ...
        for satName in self.satNames_1:
    ...
    def testRun2(self):
        actual = STECEF2AERSatelliteReader(self.sio_2, lat="ref_lat", lon="ref_lon",
        ...
        self.assertIsInstance(
            actual.loc[0, "geometry"], Point, "Test geometry column")
        self.assertEqual(actual.loc[0, "nsat"], 21, "Test nsat value")
        self.assertTrue(
            (actual.loc[0, "timeUTC"] - self.dt_2) < timedelta(seconds=1), "Test timeUTC value")

        for i in range(SatelliteLib.NSAT):
            self.assertTrue(f"{SatelliteLib.get_satellite_name(i)}_az" in actual, "Test column names")
            self.assertTrue(f"{SatelliteLib.get_satellite_name(i)}_el" in actual, "Test column names")
            self.assertTrue(f"{SatelliteLib.get_satellite_name(i)}_sr" in actual, "Test column names")

        actual.dropna(axis=1, inplace=True)
        for sat in ["E02", "E07", "E18", "E27", "E29", "E30", "E36",
                    "R07", "R08", "R09", "R10", "R16", "R17", "R18", "R19",
                    "G06", "G12", "G24", "G25", "G28", "G32"]:
            self.assertTrue(f"{sat}_az" in actual, "Test column names")
            self.assertTrue(f"az_{sat}" in actual, "Test column names")
            
            self.assertTrue(f"{sat}_el" in actual, "Test column names")
            self.assertTrue(f"el_{sat}" in actual, "Test column names")
            
            self.assertTrue(f"{sat}_sr" in actual, "Test column names")
            # self.assertTrue(f"sr_{sat}" in actual, "Test column names")
