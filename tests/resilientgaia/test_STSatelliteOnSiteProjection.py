...
""")
        self.sat_2 = STECEF2AERSatelliteReader(self.sio_2, lat="ref_lat", lon="ref_lon",
                                               alt="ref_alt", sep=",", decimal=".",
                                               iepsg="epsg:4326", oepsg="epsg:2154",
                                               timestampFieldName="timeUTC",
                                               tzinfo=timezone("Europe/Paris")).run()
        self.sat_2["gid"] = range(len(self.sat_2))
        self.dt_2 = to_datetime("2024-12-19 14:38:01+00:00")
    ...
    def testRun1(self):
        actual = STSatelliteOnSiteProjection(self.sat_1, gid="gid",
                                             timestamp="timeUTC",
                                             proj="Stereographic", size=1).run()

        self.assertIsInstance(actual, GeoDataFrame, "Is a GeoDataFrame")
        self.assertEqual(len(actual), 22, "Count rows")
        self.assertEqual(len(actual.columns), 7, "Count columns")

        self.assertEqual(len(actual.query("constellation == 'GPS'")), 9, "Count GPS")
        self.assertEqual(len(actual.query("constellation == 'GLONASS'")), 6, "Count GLONASS")
        self.assertEqual(len(actual.query("constellation == 'Galileo'")), 7, "Count Galileo")
        self.assertTrue(
            (actual.loc[0, "timeUTC"] - self.dt959_1) < timedelta(seconds=1), "Test timeUTC value")

        for i, j in enumerate([4, 6, 8, 10, 12, 14, 17, 19, 29, 38, 40, 46,
                               47, 53, 54, 62, 63, 66, 72, 88, 91, 93]):
            self.assertEqual(f"sat_{j}", actual.loc[i, "satName"], "Test column names")

    def testRun2(self):
        actual = STSatelliteOnSiteProjection(self.sat_2, gid="gid",
                                             timestamp="timeUTC",
                                             proj="Stereographic", size=1).run()

        self.assertIsInstance(actual, GeoDataFrame, "Is a GeoDataFrame")
        self.assertEqual(len(actual), 21, "Count rows")
        self.assertEqual(len(actual.columns), 7, "Count columns")

        self.assertEqual(len(actual.query("constellation == 'GPS'")), 6, "Count GPS")
        self.assertEqual(len(actual.query("constellation == 'GLONASS'")), 8, "Count GLONASS")
        self.assertEqual(len(actual.query("constellation == 'Galileo'")), 7, "Count Galileo")
        self.assertTrue(
            (actual.loc[0, "timeUTC"] - self.dt_2) < timedelta(seconds=1), "Test timeUTC value")
