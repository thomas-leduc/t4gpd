...
from t4gpd.wrf.SkyMapRadiationBalance import SkyMapRadiationBalance


class SkyMapRadiationBalanceTest(unittest.TestCase):
    def setUp(self):
...
"""
    ...
    def testRun1(self):
        op = SkyMapRadiationBalance(self.skymaps, self.day, meteo=None)
        ...
        self.assertEqual(len(self.sensors.columns) + 14, len(actual.columns), "Count columns")
        self.assertTrue(actual.sw_direct.apply(lambda v: 2100 < v < 3200).all(), "Test sw_direct attribute values")
        self.assertTrue(actual.sw_diffuse.apply(lambda v: 300 < v < 700).all(), "Test sw_diffuse attribute values")
        # self.__plot(actual)

    def testRun2(self):
        op = SkyMapRadiationBalance(self.skymaps, self.day, meteo=self.meteo)
        actual = STGeoProcess(op, self.skymaps).run()
        self.assertIsInstance(actual, GeoDataFrame, "Is a GeoDataFrame")
        self.assertEqual(len(self.sensors), len(actual), "Count rows")
        self.assertEqual(len(self.sensors.columns) + 14, len(actual.columns), "Count columns")
        self.assertTrue(actual.sw_direct.apply(lambda v: 2400 < v < 3200).all(), "Test sw_direct attribute values")
        self.assertTrue(actual.sw_diffuse.apply(lambda v: 300 < v < 700).all(), "Test sw_diffuse attribute values")
        # self.__plot(actual)
