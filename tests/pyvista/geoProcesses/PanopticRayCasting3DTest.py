...
class PanopticRayCasting3DTest(unittest.TestCase):
    ...
    def testRun1(self):
        nrays = 32
        op = PanopticRayCasting3D([self.gdf], maskPkFieldname=None, nrays=nrays,
                                  method='geodeciel', maxRayLen=15, encode=False)
        ...
        # scene.plot(show_edges=True, point_size=10.0, render_points_as_spheres=True)

    def testRun2(self):
        nrays = 8
        op = PanopticRayCasting3D([self.gdf], maskPkFieldname='gid', nrays=nrays,
                                  method='geodeciel', maxRayLen=15, encode=False)
        result = STGeoProcess(op, self.sensors).execute()
        ...
        for _, row in result.iterrows():
            ...
            for hitGid in row.hitGids:
                if 0 == row.gid:
                    self.assertIn(hitGid, [None, 0, 369], 'Check hitGids attribute value')
                elif 100 == row.gid:
                    self.assertIn(hitGid, [None, 100], 'Check hitGids attribute value')
                elif 200 == row.gid:
                    self.assertIn(hitGid, [None, 200], 'Check hitGids attribute value')
                elif 300 == row.gid:
                    self.assertIn(hitGid, [None, 300], 'Check hitGids attribute value')
