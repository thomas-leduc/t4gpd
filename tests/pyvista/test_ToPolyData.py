...
class ToPolyDataTest(unittest.TestCase):

    def setUp(self):
        seed(0)
        buildings = GeoDataFrameDemos.regularGridOfPlots(3, 5, dw=5.0)
    ...
    def testRun1(self):
        ...
        result = ToPolyData([self.buildingsIn3d, self.roads, self.sensors], 'MonAttribut').run()
        result.plot(scalars='MonAttribut', cmap='gist_earth', show_edges=False,
                    ...
                    screenshot='./my_pivista_1.png')

    def testRun2(self):
        bLayer = ToPolyData([self.buildingsIn3d], 'HAUTEUR').run()
        rLayer = ToPolyData([self.roads]).run()
        sLayer = ToPolyData([self.sensors], 'MonAttribut').run()
        ...
        plotter.show(screenshot='./my_pivista_2.png')

    def testRun3(self):
        building = GeoDataFrameDemos.singleBuildingInNantes()
        result = ToPolyData([building]).run()
        result.plot(cmap='gist_earth', show_edges=True,
                    screenshot='./my_pivista_3.png')
