...
from t4gpd.graph.STDegreeCentrality import STDegreeCentrality


class STDegreeCentralityTest(unittest.TestCase):
    ...
    def __plot(self, result):
        ...
        result.apply(lambda x: ax.annotate(
            text=f"{x.gid} ({x.valency})\n{x.degree_c:.3f}", xy=x.geometry.coords[0], color="black",
    ...
    def testRun(self):
        result = STDegreeCentrality(self.roads).run()
        ...
        for _, row in result.iterrows():
            ...
            self.assertTrue(0 <= row["degree_c"] <= 1,
