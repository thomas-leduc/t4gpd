...
class STBetweennessCentralityTest(unittest.TestCase):
    ...
    def __plot(self, result):
        ...
        result.apply(lambda x: ax.annotate(
            text=f"{x.gid} ({x.valency})\n{x.betweenness:.3f}", xy=x.geometry.coords[0], color="black",
    ...
    def testRun(self):
        result = STBetweennessCentrality(self.roads).run()
        ...
        for _, row in result.iterrows():
            ...
            self.assertTrue(0 <= row["betweenness"] <= 1,
                            "betweenness attribute values")
        # self.__plot(result)
