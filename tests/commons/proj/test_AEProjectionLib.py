...
class AEProjectionLibTest(unittest.TestCase):
    ...
    def test_projection_switch(self):
        ...
        for projectionName, expected in PAIRS:
            result = AEProjectionLib.projection_switch(
                projectionName)
    ...
    def test_reverse_projection_switch(self):
        ...
        for projectionName, expected in PAIRS:
            result = AEProjectionLib.reverse_projection_switch(
                projectionName)
            self.assertEqual(expected, result.__name__,
    ...
    def test_orthogonal_projection(self):
        ...
        for azim, elev, expected in trios:
            actual = AEProjectionLib.orthogonal_projection(
                self.vp, azim, elev, size=1)
            self.assertAlmostEqual(expected.distance(
                Point(actual)), 0, None, "Test orthogonal projection", self.epsilon)
    ...
    def test_stereographic_projection(self):
        ...
        for azim, elev, expected in trios:
            actual = AEProjectionLib.stereographic_projection(
                self.vp, azim, elev, size=1)
            self.assertAlmostEqual(expected.distance(
                Point(actual)), 0, None, "Test stereographic projection", self.epsilon)
