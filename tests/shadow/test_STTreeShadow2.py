...
from t4gpd.shadow.STTreeShadow2 import STTreeShadow2


class STTreeShadow2Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.trees["h_houppier"] = self.trees.osm_id.apply(
            lambda _id: 2 * (a1 if _id in ids else a2)
        )
        self.trees["up_rad"] = self.trees.osm_id.apply(
            lambda _id: a1 if _id in ids else a2
        )
        self.trees["down_rad"] = self.trees.osm_id.apply(
            lambda _id: a1 if _id in ids else a2
        )
