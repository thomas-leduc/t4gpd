...
class STCoolscapesTessellationTest(unittest.TestCase):
    ...
    def testRun1(self):
        # lines = gpd.read_file('../data/jardin_extraordinaire_path.shp').to_crs('EPSG:2154')
        lines = TestUtils.loadDataSet('../data', 'jardin_extraordinaire_path.shp').to_crs('EPSG:2154')
    ...
    def testRun2(self):
        # lines = gpd.read_file('../data/jardin_extraordinaire_path5.shp').to_crs('EPSG:2154')
        lines = TestUtils.loadDataSet('../data', 'jardin_extraordinaire_path5.shp').to_crs('EPSG:2154')
        sampleDist, buffDist = 3.0, 1.0 
        tessell = STCoolscapesTessellation(lines, sampleDist, buffDist).run()
