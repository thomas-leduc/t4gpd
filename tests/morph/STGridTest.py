...
class STGridTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        countries = read_file(get_path('naturalearth_lowres'))
        countries = countries.query('"France" == name')
        wgs84_roi = Polygon([(-10, 40), (-10, 52), (10, 52), (10, 40)])
        countries = clip(countries, wgs84_roi)
        self.countries = countries.to_crs("EPSG:2154")
    ...
    def __plot(self, inputGdf, grid):
        ...
        if (0 < len(grid['Point' == grid.geom_type])):
            # NODES
            if 'indoor' in grid:
                grid.plot(ax=basemap, column='indoor', legend=True, cmap='plasma')
            else:
                grid.plot(ax=basemap)
        else:
            # POLYGONS
            if 'indoor' in grid:
                grid.plot(ax=basemap, edgecolor='black', alpha=0.75, column='indoor', legend=True, cmap='plasma')
            else:
    ...
    def __commons(self, inputGdf, grid, nFeatures, nIndoorFeatures=0, nOutdoorFeatures=0):
        ...
        if (0 < nIndoorFeatures) or (0 < nOutdoorFeatures):
            self.assertEqual(nIndoorFeatures, len(grid.query('True == indoor')), "Indoor features count")
            self.assertEqual(nOutdoorFeatures, len(grid.query('False == indoor')), "Outdoor features count")
            if 0 == nIndoorFeatures:
                ...
                for indoor in grid['indoor']:
                    self.assertIn(indoor, [True, False], "Indoor or outdoor grid items")

    def testRunBuildings_Point_None(self):
        grid = STGrid(self.buildings, 50, dy=None, indoor=None, intoPoint=True).run()
        self.__commons(self.buildings, grid, 25)
        # self.__plot(self.buildings, grid)

    def testRunBuildings_Point_Both(self):
        grid = STGrid(self.buildings, 50, dy=None, indoor='both', intoPoint=True).run()
        self.__commons(self.buildings, grid, 25, 10, 15)
        # self.__plot(self.buildings, grid)

    def testRunBuildings_Point_True(self):
        grid = STGrid(self.buildings, 50, dy=None, indoor=True, intoPoint=True).run()
        self.__commons(self.buildings, grid, 10, 10, 0)
        # self.__plot(self.buildings, grid)

    def testRunBuildings_Point_False(self):
        grid = STGrid(self.buildings, 50, dy=None, indoor=False, intoPoint=True).run()
        self.__commons(self.buildings, grid, 15, 0, 15)
        # self.__plot(self.buildings, grid)

    def testRunBuildings_Polygon_None(self):
        grid = STGrid(self.buildings, 50, dy=None, indoor=None, intoPoint=False).run()
        self.__commons(self.buildings, grid, 25)
        # self.__plot(self.buildings, grid)

    def testRunBuildings_Polygon_Both(self):
        grid = STGrid(self.buildings, 50, dy=None, indoor='both', intoPoint=False).run()
        self.__commons(self.buildings, grid, 25, 10, 15)
        # self.__plot(self.buildings, grid)

    def testRunBuildings_Polygon_True(self):
        grid = STGrid(self.buildings, 50, dy=None, indoor=True, intoPoint=False).run()
        self.__commons(self.buildings, grid, 10, 10, 0)
        # self.__plot(self.buildings, grid)

    def testRunBuildings_Polygon_False(self):
        grid = STGrid(self.buildings, 50, dy=None, indoor=False, intoPoint=False).run()
        self.__commons(self.buildings, grid, 15, 0, 15)
        # self.__plot(self.buildings, grid)
    ...
    def testRunCountries_Point_None(self):
        grid = STGrid(self.countries, 2e5, dy=None, indoor=None, intoPoint=False).run()
        self.__commons(self.countries, grid, 6 * 6)
        # self.__plot(self.countries, grid)
