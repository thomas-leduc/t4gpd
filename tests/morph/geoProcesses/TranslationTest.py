...
class TranslationTest(unittest.TestCase):
    ...
    def testRun(self):
        for vector in [Point(111, 222), (111, 222), [111, 222]]:
            result = STGeoProcess(Translation(vector), self.inputGdf).run()
            ...
            for _, row in result.iterrows():
                if (0 == row['gid']):
                    self.assertEqual(row.geometry, Point((111, 222)), 'Equality test (1)')
                elif (1 == row['gid']):
                    self.assertEqual(row.geometry, Point((211, 222)), 'Equality test (2)')
                elif (2 == row['gid']):
                    self.assertEqual(row.geometry, Point((111, 322)), 'Equality test (2)')
