...
class OrientedSVFTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.viewpoints = STGrid(self.buildings, dx=80, indoor=False,
                                 intoPoint=True).run()
        ...
        self.skymaps = STSkyMap25D(
            self.buildings, self.viewpoints, nRays=nRays, rayLength=100,
    ...
    def __plot(self, actual, method):
        ...
        self.skymaps.apply(lambda x: ax.annotate(
    ...
    def testRun1(self):
        ...
        self.assertEqual(len(self.skymaps.columns)+4,
                         len(actual.columns), "Count columns")
        for _, row in actual.iterrows():
            avgSvf = (row.svf4_north + row.svf4_east +
                      row.svf4_south + row.svf4_west) / 4
            self.assertAlmostEqual(
                row.svf, avgSvf, None, "Test svf* values", 1e-9)
    ...
    def testRun2(self):
        ...
        self.assertIsInstance(actual, GeoDataFrame, "Is a GeoDataFrame")
        self.assertEqual(len(self.skymaps), len(actual), "Count rows")
        self.assertEqual(len(self.skymaps.columns)+8,
                         len(actual.columns), "Count columns")
        for _, row in actual.iterrows():
            avgSvf = (row.svf8_north + row.svf8_northeast +
                      row.svf8_east + row.svf8_southeast +
                      row.svf8_south + row.svf8_southwest +
                      row.svf8_west + row.svf8_northwest) / 8
            self.assertAlmostEqual(
                row.svf, avgSvf, None, "Test svf* values", 1e-9)
    ...
    def testRun3(self):
        ...
        self.assertIsInstance(actual, GeoDataFrame, "Is a GeoDataFrame")
        self.assertEqual(len(self.skymaps), len(actual), "Count rows")
        self.assertEqual(len(self.skymaps.columns)+12,
                         len(actual.columns), "Count columns")
        for _, row in actual.iterrows():
            avgSvf = (row.svf12_n + row.svf12_nne +
                      row.svf12_ene + row.svf12_e +
                      row.svf12_ese + row.svf12_sse +
                      row.svf12_s + row.svf12_ssw +
                      row.svf12_wsw + row.svf12_w +
                      row.svf12_wnw + row.svf12_nnw) / 12
            self.assertAlmostEqual(
                row.svf, avgSvf, None, "Test svf* values", 1e-9)
    ...
    def testRun4(self):
        ...
        self.assertIsInstance(actual, GeoDataFrame, "Is a GeoDataFrame")
        self.assertEqual(len(self.skymaps), len(actual), "Count rows")
        self.assertEqual(len(self.skymaps.columns)+4+12,
                         len(actual.columns), "Count columns")
        ...
        for _, row in actual.iterrows():
            ...
            epsilon = 1e-9
            self.assertAlmostEqual(row.svf4_north, north,
                                   None, "Test north values", epsilon)
            self.assertAlmostEqual(row.svf4_east, east,
                                   None, "Test east values", epsilon)
            self.assertAlmostEqual(row.svf4_south, south,
                                   None, "Test south values", epsilon)
            self.assertAlmostEqual(row.svf4_west, west,
                                   None, "Test west values", epsilon)
