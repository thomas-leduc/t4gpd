...
class DoubleProjectionOfWallsLib(object):
    ...
    CIRCLE = LinearRing(zip(cos(linspace(0, 2 * pi, 90)), sin(linspace(0, 2 * pi, 90))))
    NO_IDX = -123456789
    ...
    @staticmethod
    def __double_projection_of_LineString(
        prj, viewpoint, lineStringMask, size, npts, circle
    ):
        circle = translate(circle, xoff=viewpoint.x, yoff=viewpoint.y)

        pts = [
            lineStringMask.interpolate(x, normalized=True).coords[0]
            for x in linspace(0, 1, npts)
        ]
        ...
        if curvA < curvB:
            if 0.9 <= 1 - curvB + curvA:
                ...
                return None
            arc1 = substring(circle, start_dist=curvB, end_dist=1, normalized=True)
            arc2 = substring(circle, start_dist=0, end_dist=curvA, normalized=True)
            arcPts = [(p[0], p[1], viewpoint[2]) for p in arc1.coords] + [
                (p[0], p[1], viewpoint[2]) for p in arc2.coords
            ]
        else:
            arc = substring(circle, start_dist=curvB, end_dist=curvA, normalized=True)
    ...
    def __foo(reverse_prj, size, masks, rayLength):
        ...
        for vppk in vppks:
            masks2 = masks.loc[
                masks[vppk == masks.__VIEWPOINT_PK__].index,
                [
                    "__MASK_IDS__",
                    "__VIEWPOINT_PK__",
                    "__VIEWPOINT_GEOM__",
            ...
            masks3["__REPRESENTATIVE_POINT__"] = masks3.geometry.apply(
                lambda geom: GeomLib.forceZCoordinateToZ0(
                    geom.representative_point(), z0=vpgeom.z
                )
            )
            masks3["__MASK_AREA__"] = masks3.geometry.apply(lambda geom: geom.area)
        ...
        masks4["__1ST_FRONT__"] = masks4.apply(
            lambda row: DoubleProjectionOfWallsLib.__managing_overlapping_fronts(
                row.__VIEWPOINT_GEOM__,
                row.__REPRESENTATIVE_POINT__,
                row.__WALL_MASK__,
                rayLength,
            ),
            axis=1,
        ...
        def __get_id(ids, mid):
            return (
                f"ERR_{vppk}"
                if (DoubleProjectionOfWallsLib.NO_IDX == mid)
                else ids[mid]
            )

        masks4["__MASK_DIST__"] = masks4.__1ST_FRONT__.apply(lambda v: v[0])
        masks4["__MASK_ID__"] = masks4.apply(
            lambda row: __get_id(row.__MASK_IDS__, row.__1ST_FRONT__[1]), axis=1
        )

        def __get_wall_mask(wall_masks, mid):
            return (
                LineString([])
                if (DoubleProjectionOfWallsLib.NO_IDX == mid)
                else wall_masks[mid]
        ...
        masks4["__WALL_MASK__"] = masks4.apply(
            lambda row: __get_wall_mask(row.__WALL_MASK__, row.__1ST_FRONT__[1]), axis=1
        )

        masks4 = masks4.dissolve(
            by=["__VIEWPOINT_PK__", "__MASK_ID__", "__WALL_MASK__"],
            aggfunc={
                "__VIEWPOINT_GEOM__": "first",
                "__MASK_DIST__": list,
                "__MASK_AREA__": list,
            },
            as_index=True,
        ...
        def __get_dist(dists, areas):
            return dot(asarray(areas), asarray(dists)) / sum(asarray(areas))

        masks4["__DEPTH__"] = masks4.apply(
            lambda row: __get_dist(row.__MASK_DIST__, row.__MASK_AREA__), axis=1
        )
    ...
    @staticmethod
    def __managing_overlapping_fronts(
        viewpoint, representative_point, wall_masks, rayLength
    ):
        vp, rp, d = (
            viewpoint,
            representative_point,
            viewpoint.distance(representative_point),
        )
        ray = LineString(
            [
                (vp.x, vp.y),
    ...
    def walls(
        ...
        reverse_prj = DoubleProjectionLib.reverse_projection_switch(projectionName)
        ...
        circle = scale(
            DoubleProjectionOfWallsLib.CIRCLE, xfact=size, yfact=size, origin=(0, 0)
        )
        ...
        masks = DoubleProjectionOfWallsLib.__foo(reverse_prj, size, masks, rayLength)
        # masks.to_csv("/tmp/5.csv", index=False, sep=";")

        # POST-PROCESSING
        # masks.drop(columns=["__VIEWPOINT__"], inplace=True)
        if aggregate:
            masks.__VIEWPOINT_GEOM__ = masks.__VIEWPOINT_GEOM__.apply(lambda g: g.wkt)
            masks = masks.dissolve(
                by=["__VIEWPOINT_PK__", "__VIEWPOINT_GEOM__"], as_index=True
            )
            masks.geometry = masks.geometry.apply(lambda geom: geom.buffer(0))
            ...
            if "isoaire" == projectionName.lower():
                tmp = pi * size**2
                masks["svf"] = masks.geometry.apply(lambda geom: geom.area / tmp)
        else:
        ...
        if encode:
            ...
            if "__WALL_MASK__" in masks:
                masks.__WALL_MASK__ = masks.__WALL_MASK__.apply(lambda g: g.wkt)
    ...
    def test(ofile=None):
        ...
        from t4gpd.commons.proj.DoubleProjectionOfPointsLib import (
            DoubleProjectionOfPointsLib,
        )
        ...
        ax.set_title(f"Montpellier (34), IRIS Tastavin ({projectionName})", size=28)
