...
class DoubleProjectionOfSpheresLib(object):
    ...
    @staticmethod
    def __projection_of_tree_crown(
        prj, viewpoint, treeCrownCenter, treeCrownRadius, size, npts
    ):
        ...
        pts = DoubleProjectionOfSpheresLib.__tree_crown_mask_contour(O, A, B, npts)
    ...
    def trees(
        ...
        # PRE-PROCESSING
        result = CartesianProductLib.product_within_distance2(sensors2, trees, horizon)
        ...
        # POST-PROCESSING
        result["depth"] = result.geometry.apply(lambda geom: geom.exterior.coords[0][2])
    ...
    def test():
        ...
        coursCambronne = GeoDataFrameDemos.coursCambronneInNantes()
        trees = GeoDataFrameDemos.districtGraslinInNantesTrees()
        trees = clip(trees, coursCambronne)
        ...
        minx, miny, maxx, maxy = coursCambronne.total_bounds
