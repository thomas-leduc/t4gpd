...
class MyNode(object):
    ...
    def __init__(self, *args):
        ...
        if (1 == len(args)) and isinstance(args[0], Point):
            ...
            self.x, self.y, self.z = (
                list(args[0]) + [0.0] if (2 == len(args[0])) else args[0]
            )

        elif (
            (2 <= len(args)) and MyNode.isnumeric(args[0]) and MyNode.isnumeric(args[1])
        ):
            ...
            raise RuntimeError(
                "Initialization error with %s of type %s" % (args, type(args))
            )
        self.hash = None
    ...
    def __eq__(self, other):
        ...
        result = (
            isinstance(other, MyNode)
            and Epsilon.equals(selfx, otherx)
            and Epsilon.equals(selfy, othery)
            and Epsilon.equals(selfz, otherz)
        )
        return result
