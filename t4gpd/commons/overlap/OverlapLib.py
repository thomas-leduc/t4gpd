...
class OverlapLib(object):
    ...
    def overlap(gdf, fieldnames, patchid="__PATCH_ID__"):
        ...
        for fieldname in fieldnames:
            if fieldname not in gdf:
                raise IllegalArgumentTypeException(fieldname, "fieldname of gdf")
            # if ((pk not in gdf) or (pk in gdf) and (not gdf[pk].is_unique)):
            # raise IllegalArgumentTypeException(pk, "primary key of gdf")

        if patchid in gdf:
            raise IllegalArgumentTypeException(patchid, "fieldname unknown to gdf")
        ...
        result = sjoin(representative_points, gdf, how="inner", predicate="within")
    ...
    def test():
        ...
        result = OverlapLib.overlap(gdf, ["gid", "timestamp"], patchid="patch_id")
        print(result)
        ...
        fig, axes = plt.subplots(
            nrows=1, ncols=2, figsize=(1.6 * 8.26, 0.8 * 8.26), squeeze=False
        )
        ...
        for i, patch_id in enumerate(result.patch_id.unique()):
            result[result.patch_id == patch_id].boundary.plot(ax=ax, hatch=HATCHES[i])
        ax.axis("off")
